...
///
/// ``DynamicText`` solves this problem by providing an interface similar to `SwiftUI.Text` that can
...
///
/// ```swift
/// state.label = DynamicText("Hello, ") + DynamicText(name).bold() + DynamicText("!")
/// ```
///
...
/// To convert ``DynamicText`` to `SwiftUI.Text` for this purpose, you can use the `Text` initializer:
///
/// ```swift
...
/// ``DynamicText`` under the hood. See ``AlertState`` and `ActionState` accordingly.
///
/// In the future, should `SwiftUI.Text` and `SwiftUI.LocalizedStringKey` reliably conform to
/// `Equatable`, ``DynamicText`` may be deprecated.
///
/// - Note: ``DynamicText`` does not support _all_ `LocalizedStringKey` permutations at this time
...
extension DynamicText {
  public init(
    ...
    self.init(id: id, storage: .verbatim(content))
  }
  ...
  @available(*, deprecated, message: "id property is autogenerated on concatenation")
...
extension DynamicText: CustomDumpRepresentable {
  public var customDumpValue: Any {
    func dumpHelp(_ dynamicText: Self) -> String {
      var output: String
      switch dynamicText.storage {
      case let .concatenated(lhs, rhs):
      ...
      for modifier in dynamicText.modifiers {
        switch modifier {
        ...
        case .bold, .fontWeight(.some(.bold)):
          output = "**\(output)**"
        case .font(.some):
          break  // TODO: capture Font description using DSL similar to DynamicText and print here
...
extension DynamicText: ExpressibleByStringLiteral {
  public init(stringLiteral value: String) {
    self.init(value, id: .init(rawValue: value))
  }
}

extension DynamicText: _PrimitiveDynamicElement {
